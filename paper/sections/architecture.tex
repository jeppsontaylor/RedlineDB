...
visibility CSN. Free space within a page is reclaimed via vacuuming
during the next compaction pass. The on-disk footprint is reported
through a recursive walker (the bench harness uses \texttt{walkdir}
to sum every byte under the database directory, including WAL
segments, catalog snapshots, and the rolled-archive segment store).
...
$(\mathrm{table\_id}, \mathrm{row\_id})$, never just $\mathrm{row\_id}$).
...
before returning a \texttt{BUSY}-class error. The
